...
const Sorter = observer(({productListType} : {productListType : ProductListType}) => {
  const [activeLanguageKey, setActiveLanguageKey] = useState<SortKey>("name:asc");
        ...
        {({ open, close }) => (
          <>
            <Popover.Button className="flex w-full items-center justify-between gap-2 rounded border border-primary/15 px-5 py-2.5 dark:border-white/15 lg:min-w-60">
              ...
              <span className={`flex size-4 items-center justify-center transition-transform ${open ? 'rotate-180' : ''}`}>
                <MdKeyboardArrowDown />
              </span>
            </Popover.Button>
            <Transition
