...
const MyOrderTable = ({filteredData , appliedFilter} : {filteredData: Order[] , appliedFilter: Filter}) => {
  ...
  const filteredCartItems = useMemo(() => {
    if (!selectedOrder || !searchTerm) return selectedOrder?.["Cart Details"] || [];
    
    const term = searchTerm.toLowerCase();
    return selectedOrder["Cart Details"].filter(item => 
  ...
  return (
    <>
        ...
        <Modal isOpen={isModalOpen} onClose={closeModal} className="max-w-6xl h-[460px]">
            {selectedOrder && (
            <>
                <div className="text-md p-4 flex justify-between items-center">
                <div className="flex gap-6">
                    <div>SAP Order # : <span className="text-green-800 font-semibold">{selectedOrder["SAP Order"]}</span></div>	
                    <div>OMS Order # : <span className="text-green-800 font-semibold">{selectedOrder["OMS Order"]}</span></div>
                </div>
                ...
                <table className="w-full">
                    ...
                    {filteredCartItems.length > 0 ? (
                        ...
                        <tr key={product.sku} className={`${appliedFilter.sku === product.sku ? "bg-yellow-200" : ""}`}>
                        ...
                        <td className="px-4 py-2 xl:w-40 text-sm font-medium text-gray-900 dark:text-white">
                            ${numberFormatter(parseFloat(((product.quantity || 1) * product.price).toFixed(2)))}
                        </td>
                        </tr>
                    ))) : 
                    (
                        <tr>
                        <td colSpan={6} className="px-4 py-32 text-center text-gray-500 dark:text-gray-400">
                            {searchTerm ? "No product found" : "No products available"}
                        </td>
                        </tr>
                    )}
                    </tbody>
                ...
                <div>Total SKUS : <span className="text-blue-700 font-semibold">{numberFormatter(selectedOrder["Total SKUs"])}</span></div>
                <div>Total Quantity : <span className="text-blue-700 font-semibold">{numberFormatter(selectedOrder["Order Quantity"])}</span></div>
                <div>Open Quantity : <span className="text-yellow-500 font-semibold">{numberFormatter(selectedOrder["Total Back Order"])}</span></div>
                <div>Gross Value : <span className="text-blue-700 font-semibold">${numberFormatter(selectedOrder["Gross Value"])}</span></div>
                </div>
            </>
            )}
        </Modal>
